...
PlasticTexture::PlasticTexture(const Image& textureImage) : textureImage(textureImage) {
...
MainComponent::MainComponent()
	...
	osc1OctMenu.onChange = [this] {
		updateStatus(DeviceResponse(STATUS_MESSAGES[MODIFYING_PROGRAM]));
		Thread::launch([this] {
			auto status = midiProcessor.changeOscPitch(OSC1, osc1OctMenu.getSelectedItemIndex() + 5, 
				osc1SemiMenu.getSelectedItemIndex(), osc1LFButton.getToggleState());
			MessageManager::callAsync([this, status] { updateStatus(status); });
		});
	...
	osc2OctMenu.onChange = [this] {
		updateStatus(DeviceResponse(STATUS_MESSAGES[MODIFYING_PROGRAM]));
		Thread::launch([this] {
			auto status = midiProcessor.changeOscPitch(OSC2, osc2OctMenu.getSelectedItemIndex() + 5, 
				osc2SemiMenu.getSelectedItemIndex(), osc2LFButton.getToggleState());
			MessageManager::callAsync([this, status] { updateStatus(status); });
		});
	};
	...
	osc3OctMenu.onChange = [this] {
		updateStatus(DeviceResponse(STATUS_MESSAGES[MODIFYING_PROGRAM]));
		Thread::launch([this] {
			auto status = midiProcessor.changeOscPitch(OSC3, osc3OctMenu.getSelectedItemIndex() + 5, 
				osc3SemiMenu.getSelectedItemIndex(), osc3LFButton.getToggleState());
			MessageManager::callAsync([this, status] { updateStatus(status); });
		});
	};
...
void MainComponent::updateStatus(DeviceResponse response) {
	...
	if (response.status == STATUS_MESSAGES[CONNECTED]) {
		...
		response.status == STATUS_MESSAGES[MODIFYING_PROGRAM] ? 
			statusLabel.setBounds(500, 5, 300, 30) : statusLabel.setBounds(560, 5, 300, 30);
